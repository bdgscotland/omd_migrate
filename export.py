...
class OpenMetadataExporter:
    ...
    def _apply_env_overrides(self):
        ...
        if os.getenv("EXPORT_INCLUDE_DELETED"):
            self.config["export"]["include_deleted"] = (
                os.getenv("EXPORT_INCLUDE_DELETED").lower() == "true"
            )
    ...
    def _write_ndjson(self, entities: List[Dict], filename: str, output_dir: Path):
        ...
        try:
            ...
            console.print(
                f"[green]✓[/green] Exported {len(entities)} {filename} to {filepath}"
            )
            return len(entities)
    ...
    def _export_entity_type(
        self, entity_name: str, entity_class, output_dir: Path
    ) -> int:
    ...
    def export_all(self, selected_entities: Optional[List[str]] = None):
        ...
        with Progress() as progress:
            task = progress.add_task(
                "[green]Exporting entities...", total=len(entities_to_export)
            )
...
@click.option(
    ...
    "-e",
    multiple=True,
    help="Specific entity types to export (e.g., -e data_products -e domains)",
)
@click.option(
    "--clear", is_flag=True, help="Clear existing export state before starting"
)
    ...
    try:
        ...
        if clear:
            ...
            if output_path.exists():
                console.print(
                    f"[yellow]🗑️  Clearing existing export state: {output_path}[/yellow]"
                )
