...
class TestOpenMetadataExporter:
        ...
        try:
            with patch('export.load_dotenv'):
                with patch.object(OpenMetadataExporter, '_create_client'):
                    ...
                    assert exporter.config['openmetadata']['server_url'] == 'http://test.example.com'
            ...
            with patch.dict(os.environ, {
                ...
                with patch('export.load_dotenv'):
                    with patch.object(OpenMetadataExporter, '_create_client'):
                        ...
                        assert exporter.config['openmetadata']['server_url'] == 'http://env.example.com'
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                with patch('export.load_dotenv'):
                    ...
                    count = exporter._write_ndjson(test_entities, 'test_entities', Path(temp_dir))
                    assert count == 2
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                with patch('export.load_dotenv'):
                    ...
                    mock_entity1 = Mock()
                    mock_entity1.model_dump.return_value = {'id': '1', 'name': 'Test Domain 1'}
                    mock_entity2 = Mock()
                    mock_entity2.model_dump.return_value = {'id': '2', 'name': 'Test Domain 2'}
                    ...
                    count = exporter._export_entity_type('domains', Domain, Path(temp_dir))
    ...
    def test_selective_export_entities(self):
        ...
        try:
            with patch('export.load_dotenv'):
                with patch.object(OpenMetadataExporter, '_create_client'):
                    with patch.object(OpenMetadataExporter, '_export_entity_type') as mock_export:
            ...
            with patch.dict(os.environ, {
                ...
                with patch.object(import_module, 'load_dotenv'):
                    with patch.object(OpenMetadataImporter, '_create_client'):
                        ...
                        assert importer.config['import']['input_dir'] == '/custom/import/dir'
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            test_config = {
                ...
                'export': {
                    ...
                    'selective': {'domains': [], 'linked_data_products_only': False, 'linked_assets_only': False}
                },
                'import': {
                    'input_dir': temp_dir,
                    'import_order': ['teams', 'domains']
                },
                'logging': {'level': 'INFO'}
            }
            ...
            try:
                # Mock export data
                mock_teams = [{'id': 't1', 'name': 'Team 1'}, {'id': 't2', 'name': 'Team 2'}]
